...
var demo_r_role = 's0000000014'.replace('s','r');
var demo_r_pass = '-';

// test that read-role is defined
test('read role OK', function () {
  ok(demo_r_role, 'read role exists');
});

// create engine
test('createEngine', function() {
  var e = new SQLEngine(demo_r_role,demo_r_pass,domain);
...
module('SQLEngine AJAX tests', {
  setup: function () {
    this.e = new SQLEngine(demo_r_role,demo_r_pass,domain);
  }
});
...
asyncTest('form SELECT error', 1+1, function() {
